...
def generate_qti(test_id):
    try:
        ...
        # Use testquestion_set instead of test.questions
        ...
        if not test_questions.exists():
            ...
            raise ValueError("No questions associated with this test")
        ...
        for test_question in test_questions:
            ...
            set_outcome_value_score = ET.SubElement(
                response_if,
                "qti-set-outcome-value",
                identifier="SCORE"
            )
            ...
            set_outcome_value_score_else = ET.SubElement(
                response_else,
                "qti-set-outcome-value",
                identifier="SCORE"
            )
        ...
        logger.debug(f"Generated QTI XML for test {test_id} with {test_questions.count()} questions")
        return tree
        ...
        logger.error(f"Error generating QTI for test {test_id}: {str(e)}")
        raise